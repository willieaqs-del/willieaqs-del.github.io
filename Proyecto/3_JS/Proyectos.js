fetch('7_Proyectos/Proyectos_Informacion.xlsx')
    ...
    rows.forEach(proyecto => {
      ...
      card.innerHTML = `
        <div class="project-logo">
          <img src="7_Proyectos/${logo}.png" alt="${nombre} Logo" />
        </div>
        <div class="project-image">
          <img src="7_Proyectos/${imagenPrincipal}.jpg" alt="${nombre} Imagen Principal" />
  ...
  function renderProjectDetail(proyecto) {
    ...
    detail.innerHTML = `      
      <div class="project-detail-logo">        
        <img src="7_Proyectos/${proyecto.Logo}.png" alt="${proyecto.Nombre} Logo" />        
      </div>
  ...
  function renderProjectGallery(proyecto) {
    ...
    for (let b of bloques) {
      ...
      for (let i = 0; i < b && index < fotos.length; i++, index++) {
        html += `<img src="7_Proyectos/${fotos[index]}.jpg" alt="Foto ${index + 1}" data-index="${index}" />`;
      }
      html += '</div>';
    }
  ...
  function setupModal(fotos) {
    ...
    document.getElementById('prev').onclick = () => {
      ...
      modalImg.src = `7_Proyectos/${fotos[current]}.jpg`;
    };
